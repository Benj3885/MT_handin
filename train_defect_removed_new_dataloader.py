...
def get_dataloader(data_paths, random_vials_path=None, size=256, good_per_defect=1., batch_size=64, max_defect_imgs=inf, shuffle=True, train=False, p_random_vial=0):
    ...
    if not train:
        ...
        training_data = RandomGoodDataset(data_paths, random_vials_path, transform=transform, max_defect_imgs=max_defect_imgs, p_random_vial=0)
        return torch.utils.data.DataLoader(training_data, batch_size=batch_size, shuffle=shuffle, num_workers=4)
...
if __name__ == "__main__":
    ...
    ml_client.log_artifact(log_run.info.run_id, "train_defect_removed_new_dataloader.py")
    ml_client.log_artifact(log_run.info.run_id, "settings/defect_removed.yaml")
    ml_client.log_artifact(log_run.info.run_id, "utils/dataset.py")
    ...
    for iter in (iter_pbar := tqdm(range(ops['iterations']))):
        if not init:
        ...
        p = (1 / ((ops['iterations']-1)*iter) if ops['iterations'] > 1 else 1)
        ...
        for m in (models_pbar := tqdm(range(ops['models_per_iteration']), leave=False)):
            ...
            for epoch in (epoch_pbar := tqdm(range(ops['epochs']), leave=False)):  # loop over the dataset multiple times
                ...
                with torch.no_grad():
                    ...
                    epoch_pbar.set_description((f"val_acc: {val_acc}, Epoch"))
